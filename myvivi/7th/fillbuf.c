...
static unsigned char myvivi_cur_bar[8][3];
...
static void myvivi_precalculate_bars(int input )
{
	...
	for (k = 0; k < 8; k++) {
		...
		if (is_yuv) {
			myvivi_cur_bar[k][0] = TO_Y(r, g, b);	/* Luma */
			myvivi_cur_bar[k][1] = TO_U(r, g, b);	/* Cb */
			myvivi_cur_bar[k][2] = TO_V(r, g, b);	/* Cr */
		} else {
			myvivi_cur_bar[k][0] = r;
			myvivi_cur_bar[k][1] = g;
			myvivi_cur_bar[k][2] = b;
		}
	}
...
static void myvivi_gen_twopix(unsigned char *buf, int colorpos)
{
	...
	r_y = myvivi_cur_bar[colorpos][0]; /* R or precalculated Y */
	g_u = myvivi_cur_bar[colorpos][1]; /* G or precalculated U */
	b_v = myvivi_cur_bar[colorpos][2]; /* B or precalculated V */
