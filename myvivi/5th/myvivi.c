...
static struct videobuf_queue myvivi_vb_vidquene;
...
{
	/*队列操作2: 初始化*/
	videobuf_queue_vmalloc_init(&myvivi_vb_vidquene, &myvivi_video_qops,
...
{
	int ret;
	ret = videobuf_mmap_mapper(&myvivi_vb_vidquene, vma);
	return ret;
}
					...
					struct v4l2_fmtdesc *f)
{
	...
	strlcpy(f->description, "4:2:2, packed, YUYV",sizeof("4:2:2, packed, YUYV"));
...
{
	return (videobuf_reqbufs(&myvivi_vb_vidquene, p));
}
...
{
	return (videobuf_qbuf(&myvivi_vb_vidquene, p));
}

static int myvivi_vidioc_dqbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	return (videobuf_dqbuf(&myvivi_vb_vidquene, p,
...
static int myvivi_vidioc_streamoff(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	return videobuf_streamoff(&myvivi_vb_vidquene);
}
...
static int myvivi_init(void)
{
	...
	/*2.2*/
	myvivi_device->fops = &vivi_fops;

	/*2.3*/
	myvivi_device->ioctl_ops = &myvivi_ioctl_ops;

	/*2.4 队列操作
	 * a.定义初始化一个队列(会用到一个spinlock)
	 */	
